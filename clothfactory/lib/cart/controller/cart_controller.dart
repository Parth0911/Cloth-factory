...
class CartController {
  Future<void> incrementProductCount(
    ...
    DocumentSnapshot currentCartOrder = await FirebaseFirestore.instance
        .collection("cart")
        .doc(Shared_Prefrence.getString("currentOrderId"))
        .get();
    ...
    bool checkProductExist = currentOrderMap["product_list"].any(
        (element) => element.values.contains(document["Product_Id"]) as bool);
    if (checkProductExist) {
      var element = currentOrderMap["product_list"].firstWhere(
          (k) => k.values.contains(document["Product_Id"]) as bool,
          orElse: () => {});
