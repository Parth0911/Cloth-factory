...
class HomePageProductController {
  Future<void> addProductToCart(
    ...
    String currentOrderId = Shared_Prefrence.getString("currentOrderId");
    if (currentOrderId == "N/A") {
      Map<String, dynamic> passDataToCart = {};
      currentOrderId = CustomFunction().getCurrentTimeInInt();
      Shared_Prefrence.setString("currentOrderId", currentOrderId);
      data["count"] = 1;
      passDataToCart = {
      ...
      bool checkProductExist = currentOrderMap["product_list"].any(
          (element) => element.values.contains(data["Product_Id"]) as bool);
      if (checkProductExist) {
        var element = currentOrderMap["product_list"].firstWhere(
            (k) => k.values.contains(data["Product_Id"]) as bool,
            orElse: () => {});
